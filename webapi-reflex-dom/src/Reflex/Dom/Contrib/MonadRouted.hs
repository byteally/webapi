...
data LocationInfo = LocationInfo
    ...
    -- ^ The path split on '/'.
    , _locationPathQuery    :: [(T.Text, T.Text)]
    -- ^ Query params
...
dropSegment li@(LocationInfo _ [] _ _ _) = li
dropSegment (LocationInfo uri (x:xs) qps ctx st) =
  LocationInfo uri xs qps (ctx ++ [x]) st
...
newtype RouteT t m a = RouteT
  ...
  deriving (Functor, Applicative, Monad, MonadFix, MonadHold t,
...
instance (MonadRouted t m, Monad m) => MonadRouted t (EventWriterT t w m) where
    askRI = lift askRI
    localRI f (EventWriterT a) = EventWriterT $ mapStateT (localRI f) a
    tellUri = lift . tellUri

instance NotReady t m => NotReady t (RouteT r m) where
...
withPathSegment
    ...
    => (Dynamic t ([PathSegment], [(T.Text, T.Text)]) -> m a)
    -> m a
withPathSegment f = do
  locInfo <- _routingInfoLocation <$> askRI
  top <- holdUniqDyn $ (\x -> (_locationPathSegments $ x, _locationPathQuery x))  <$> locInfo
...
withPathSegment'
    :: (Monad m, Reflex t, MonadRouted t m, MonadFix m, MonadHold t m)
    => (Dynamic t (Maybe PathSegment, [(T.Text, T.Text)]) -> m a)
    -> m a
withPathSegment' f = do
  locInfo <- _routingInfoLocation <$> askRI
  top <- holdUniqDyn $ (\x -> (headMay . _locationPathSegments $ x, _locationPathQuery x))  <$> locInfo
  localRI (\ri -> ri { _routingInfoLocation = dropSegment <$> _routingInfoLocation ri })
          (f top)
  where
    headMay []    = Nothing
...
constructReq (LocationInfo u _ qps _ st) p =
...
appendingReq (LocationInfo u _ _ ctx _) p =
